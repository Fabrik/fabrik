 ...
 * @subpackage  Fabrik.element.slug
...
class PlgFabrik_ElementSlug extends PlgFabrik_Element
{
	/**
	 * Db table field type
	 *
	...
	public function getLabel($repeatCounter = 0, $tmpl = '')
	{
		...
		if (!$params->get('slug_showlabel', true))
		{
	...
	{
		if (!$this->getParams()->get('slug_showlabel', true))
		{
	...
	public function render($data, $repeatCounter = 0)
	{
		...
		$slugData = new stdClass;
		$slugData->id = $this->getHTMLId($repeatCounter);
		$slugData->value = $params->get('slug_showlabel', true) ? $this->getValue($data, $repeatCounter) : '';

		return $layout->render($slugData);
	}

	/**
	 * Helper method to get the default value used in getValue()
	 * Unlike other elements where readonly affects what is displayed,
	 * the slug element is always read only,
	 * so get the default value.
	 *
