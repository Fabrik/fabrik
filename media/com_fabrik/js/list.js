	...
	watchOrder: function () {
		...
		hs.each(function (h) {
			h.addEvent('click', function (e) {
				...
				var i = td.getElement('img');
				var icon = td.getElement('*[class^="icon-"]');
				...
				if (this.options.singleOrdering) {
					document.id(this.options.form).getElements('.fabrikorder, .fabrikorder-asc, .fabrikorder-desc').each(function (otherH) {
						otherTH = otherH.getParent('.fabrik_ordercell')
						if (Fabrik.bootstrapped) {
							var otherIcon = otherTH.getElement('*[class^="icon-"]');
							otherIcon.className = 'icon-menu-2';
						} else {
							var i = otherTH.getElement('img');
							if (i) {
