...
$.Autocompleter = function(input, options) {
	...
	function selectCurrent() {
		...
		//Thomas 06/05/11
		if (selected.data.formatted_address)
			var v = selected.data.formatted_address;
		else
	...
	function request(term, success, failure) {
		...
		// request handler for google geocoder
			...
			options.geocoder.geocode({'address': _query,'bounds': maxBounds}, function(_results, _status) {
	...
	geo_autocomplete: function(_geocoder, _options) {
		options = $.extend({}, $.Autocompleter.defaults, {
				...
				if (_results && _status && _status == 'OK') {
					$.each(_results, function(_key, _result) {
						if (_result.geometry && _result.geometry.viewport) {
							...
							if (maxBounds.contains(_result.geometry.location))
							{
								_parsed.push({
									data: _result,
									value: _place,
			...
			formatItem: function(_data, _i, _n, _value) {
				var _src = 'http://maps.google.com/maps/api/staticmap?visible=' + _data.geometry.viewport.getSouthWest().toUrlValue() + '|' + _data.geometry.viewport.getNorthEast().toUrlValue() + '&size=' + options.mapwidth + 'x' + options.mapheight + '&maptype=' + options.maptype + '&sensor=' + (options.mapsensor ? 'true' : 'false');
